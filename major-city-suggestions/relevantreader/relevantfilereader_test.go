...
func Test_relevantfilereader_NewRelevantFileReaderCreated(t *testing.T) {
	...
	actualRR := NewRelevantFileReader(validDataPoint, validFileType, validDataset)
...
func Test_relevantfilereader_NewRelevantFileReaderWasNotCreated(t *testing.T) {
	...
	validDataPoint := "city"
	// 4. Compute state (Act)
	actualRR := NewRelevantFileReader(validDataPoint, invalidFileType, validDataset)
...
func Test_relevantfilereader_isRelevant(t *testing.T) {
	...
	// 2. Define valid arguments (Act)
	searchTerm := "tor"
	dp := results.GetDataPoint("city")

	// 3. Define mutators over the data point to create a valid datapoint (Act)
	stateMutators := dp.GetStateMutators()
	newdp := stateMutators["city"]("Toronto")

	// 4. Define expected valid output (Act)
	expectedRes := true
